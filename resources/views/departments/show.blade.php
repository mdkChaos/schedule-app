<x-admin-layout :title="__('message.departments')">
    <div class="container">
        <x-page-header :title="$department->name" :iconClass="'bi bi-diagram-3 text-warning'">
            ...
            <x-slot:right>
                <x-btn-link class="me-1 btn-outline-success" :icon="'bi-pencil'" :message="__('message.edit')" :route="route('departments.edit', $department)" />
                    ...
                    <div class="card-body">
                        ...
                        <dl class="row mb-0">
                            <div class="row align-items-center border-bottom py-2">
                                ...
                                <dd class="col-sm-8 mb-0">{{ $department->id }}</dd>
                            </div>
                            <div class="row align-items-center border-bottom py-2">
                                <dt class="col-sm-4">{{ __('message.department') }}</dt>
                                <dd class="col-sm-8 mb-0">{{ $department->name }}</dd>
                            </div>
                            <div class="row align-items-center border-bottom py-2">
                                <dt class="col-sm-4">{{ __('message.workshop') }}</dt>
                                <dd class="col-sm-8 mb-0">{{ $department->workshop->name ?? '—' }}</dd>
                            </div>
                            <div class="row align-items-center border-bottom py-2">
                                <dt class="col-sm-4">{{ __('message.factory') }}</dt>
                                <dd class="col-sm-8 mb-0">{{ $department->workshop->factory->name ?? '—' }}</dd>
                            </div>
                            <div class="row align-items-center border-bottom py-2">
                                <dt class="col-sm-4">{{ __('message.created') }}</dt>
                                <dd class="col-sm-8 mb-0">{{ $department->created_at->format('d.m.Y H:i') }}</dd>
                            </div>
                            <div class="row align-items-center py-2">
                                <dt class="col-sm-4">{{ __('message.updated') }}</dt>
                                <dd class="col-sm-8 mb-0">{{ $department->updated_at->format('d.m.Y H:i') }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
