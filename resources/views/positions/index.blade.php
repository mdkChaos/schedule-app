<x-admin-layout :title="__('message.positions')">
    <div class="container">
        <x-page-header :title="__('message.position')" :iconClass="'bi bi-person-vcard text-dark'">
            ...
            <x-slot:right>
                <x-btn-link class="btn-outline-primary" :icon="'bi-plus-lg'" :message="__('message.add')" :route="route('positions.create')" />
                <x-btn-link class="btn-outline-danger" :icon="'bi-trash3'" :message="__('message.trashed')" :route="route('positions.trashed')" />
            </x-slot:right>
        </x-page-header>
